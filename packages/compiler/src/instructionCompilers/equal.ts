...
const equal: InstructionCompiler = function (line, context) {
	...
	if (areAllOperandsIntegers(operand1, operand2)) {
		...
		return saveByteCode(context, [WASMInstruction.I32_EQ]);
	} else if (areAllOperandsFloats(operand1, operand2)) {
		context.stack.push({ isInteger: true, isNonZero: false });
		return saveByteCode(context, [WASMInstruction.F32_EQ]);
	} else {
