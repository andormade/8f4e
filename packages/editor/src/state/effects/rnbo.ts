...
export default function (state: State, events: EventDispatcher): void {
	...
	async function removeRNBODevice(name: string) {
		if (RNBODevices[name]) {
			...
			delete state.project.rnbo.patchers[name];
			events.dispatch('saveState');
		}
	}

	async function initRNBODevice(name: string) {
		if (!audioContext || !state.project.rnbo.patchers[name]) {
			return;
		}
		...
		const RNBODevice = await RNBO.createDevice({ context: audioContext, patcher: state.project.rnbo.patchers[name] });
	...
	async function initAudioContext() {
		...
		Object.keys(state.project.rnbo.patchers).forEach(name => {
			initRNBODevice(name);
		});
	}
	...
	input.addEventListener('change', event => {
		...
		reader.addEventListener('load', readerEvent => {
			...
			if (content) {
				state.project.rnbo.patchers[file.name] = JSON.parse(content);
				events.dispatch('saveState');
