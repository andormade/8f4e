...
Okay, this one is actually asked a lot. I wrote a script to hunt down the shortest available .com domains, and 8f4e was one that caught my eye because it only had digits and letters from the hexadecimal system. When I registered it, I still had no idea what I would use it for. The project was basically named after this domain name, which doesn't really mean anything. Well, actually, it does mean something, it's the UTF-8 code for the Chinese character "轎", which means "litter" (a human-powered vehicle, not cat litter or rubbish), but this has nothing to do with the project.
